...
// TransactionHandler реализует transactionpb.TransactionServiceServer
type TransactionHandler struct {
	transactionpb.UnimplementedTransactionServiceServer
	service service.TransactionService
}

// NewTransactionHandler создаёт новый gRPC handler
...
// AddTransaction обрабатывает добавление новой транзакции
...
// ListTransactions возвращает список транзакций пользователя
func (h *TransactionHandler) ListTransactions(ctx context.Context, req *transactionpb.ListTransactionsRequest) (*transactionpb.ListTransactionsResponse, error) {
	...
	for _, t := range transactions {
		...
		if !ok {
			enumValue = 0 // TRANSACTION_TYPE_UNSPECIFIED
		}
