...
// DeleteTransaction — удаление транзакции по id
...
// GetBalance — получение баланса пользователя по user_id из пути
func (h *TransactionHandler) GetBalance(c *gin.Context) {
	userID := c.Param("id")
	if userID == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "user_id path parameter is required"})
		return
	}
