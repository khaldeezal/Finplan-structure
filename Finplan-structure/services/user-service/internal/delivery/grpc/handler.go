...
// UserHandler реализует gRPC интерфейс UserServiceServer и содержит бизнес-логику для работы с пользователями.
...
// NewHandler создает и возвращает новый экземпляр UserHandler с внедренным слоем бизнес-логики и логгером.
func NewHandler(svc service.UserService, logger *zap.Logger) userpb.UserServiceServer {
	return &UserHandler{svc: svc, logger: logger}
}

// GetUserProfile обрабатывает gRPC-запрос на получение профиля пользователя по userID.
...
// UpdateUserProfile обрабатывает gRPC-запрос на обновление профиля пользователя.
// Обновляет поля Name, Email, Currency и Language. Возвращает успешность операции и ошибку, если она возникла.
